...
std::string modFileBaseName("frame_mod");
		...
		const mavconn_mavlink_msg_container_t* container, void* user)
{
   ...
   // read mono image data
	if (client->readMonoImage(msg, img))
	{
      ...
      cv::drawKeypoints(img, kp, img_kp, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT );
	...
	case 'f':
	{
      ...
      cv::imwrite(std::string(modFileBaseName+index+fileExt).c_str(), img_kp);
	}
	break;
	default:
		break;
